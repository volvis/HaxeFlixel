...
public class FlxKongregate 
{
	...
	private static var shadowAPI:String;
	...
	public function new() 
	{
		hasLoaded = false;
		isLocal = false;
		shadowAPI = "http://www.kongregate.com/flash/API_AS3_Local.swf";
	}
	 ...
	 */
	public static function init(callbackFunc:Dynamic):Void
	{
		try
		{
			var parameters:Dynamic = FlxG.stage.loaderInfo.parameters;
		}
		catch (e:Error)
		{
		...
		if (Reflect.hasField(parameters, "kongregate_api_path"))
		{
			Security.allowDomain(Reflect.field(parameters, "kongregate_api_path"));
			apiPath = Reflect.field(parameters, "kongregate_api_path");
		}
		else
		{
	...
	private static function apiLoadComplete(event:Event):Void
	{
		...
		{
			//loadCallback.call();
			Reflect.callMethod(this, Reflect.field(this, "loadCallback"), []);
		}
	 ...
	 */
	public static function addEventListener(contentType:String, callbackFunc:Dynamic):Void
	{
		api.services.addEventListener(contentType, callbackFunc);
	}
	 ...
	 */
	public static function purchaseItem(items:Array, callbackFunc:Dynamic):Void
	{
		api.mtx.purchaseItems(items, callbackFunc);
	}
	 ...
	 * @param	callback	The callback function
	 */
	public static function requestUserItemList(username:String, callbackFunc:Dynamic):Void
	{
		api.mtx.requestUserItemList(username, callbackFunc);
	}
	 ...
	 */
	public static function saveSharedContent(type:String, content:String, callbackFunc:Dynamic, ?thumb:DisplayObject = null, ?label:String = null):Void
	{
		api.sharedContent.save(type, content, callbackFunc, thumb, label);
	}
	 ...
	 * @param	callback	Function to call when content load request has been made
	 */
	public static function submitAvatar(avatar:DisplayObject, callbackFunc:Dynamic):Void
	{
		api.images.submitAvatar(avatar, callbackFunc);
	}
}